...
int main() {
    ...
    ALLEGRO_BITMAP* historia4 = al_load_bitmap("C:\\Users\\Leon\\source\\repos\\JogoEmAllegro\\JogoEmAllegro\\historia do jogo\\PO4.png");
    ALLEGRO_BITMAP* historia5 = al_load_bitmap("C:\\Users\\Leon\\source\\repos\\JogoEmAllegro\\JogoEmAllegro\\historia do jogo\\PO5.png");
    ...
    float dino_x = 100;
    float dino_y = ALTURA_TELA - al_get_bitmap_height(dino_pulo) - 10;
    ...
    while (!sair) {
        ...
        if (evento.type == ALLEGRO_EVENT_TIMER) {
                ...
                if (!no_chao) {
                    if (espaco_pressionado && velocidade_y > PULO_FORCA * 1.2) {
                        velocidade_y = PULO_FORCA * 1.2;  // Aumenta a força do pulo ao segurar espaço

                    }
                    velocidade_y += GRAVIDADE;
                    dino_y += velocidade_y;

                    if (dino_y >= ALTURA_TELA - al_get_bitmap_height(dino_pulo) - 10) {
                        dino_y = ALTURA_TELA - al_get_bitmap_height(dino_pulo) - 10;
                        no_chao = true;
                    }
                }
        ...
        if (evento.type == ALLEGRO_EVENT_MOUSE_BUTTON_DOWN && evento.mouse.button == 1) {
            ...
            if (evento.type == ALLEGRO_EVENT_KEY_DOWN) {
                if (evento.keyboard.keycode == ALLEGRO_KEY_SPACE) {
                    if (no_chao) { // Só permite pular se estiver no chão
                        no_chao = false;
                        espaco_pressionado = true; // Marca que a tecla está pressionada
                        velocidade_y = -PULO_FORCA; // Aplica força inicial do pulo
                    }
                }
            }
